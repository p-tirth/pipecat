...
# ---------------------------------------------------------------------------
# Per-transport settings capabilities
#
# Flux exposes a different set of runtime-updatable settings on each transport,
# so every field is classified per transport: sent over the live connection
# (Configure), applied by reconnecting, applied locally, or reported as
# unsupported.
# ---------------------------------------------------------------------------
...
def test_websocket_classifies_every_setting():
    """No setting on the websocket transport is left unclassified.
    ...
    classified = (
        DeepgramFluxSTTService._CONFIGURE_FIELDS
        | DeepgramFluxSTTService._CONNECTION_FIELDS
        | DeepgramFluxSTTService._LOCAL_FIELDS
        | _UNSUPPORTED_FIELDS
    )
...
def test_sagemaker_applies_nothing_to_a_running_session():
    """The SageMaker Flux build takes no setting on a live session.

    It accepts only CloseStream as a client message and rejects `numerals` in
    the connection URL, and `model` is fixed when the endpoint is deployed.
    """
    assert DeepgramFluxSageMakerSTTService._CONFIGURE_FIELDS == frozenset()
    assert DeepgramFluxSageMakerSTTService._CONNECTION_FIELDS == frozenset()
